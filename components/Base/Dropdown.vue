<template>
    <div ref="dropdownRef" class="w-full relative flex flex-col gap-1">
        <!-- Label -->
        <label v-if="label"
            class="label text text-3xl font-semibold text-secondary-content break-words whitespace-normal leading-tight">
            {{ label }}
        </label>

        <!-- Trigger Button -->
        <button type="button" @click="toggleDropdown" :class="[
            'btn btn-outline text-3xl py-7 px-6 w-full flex justify-between items-center font-normal hover:border-secondary',
            error ? 'border-error' : ''
        ]">
        ...
        <ul v-if="isOpen"
            ...
            <li v-for="(option, index) in options" :key="index" class="hover:bg-secondary rounded-lg">
                <div @click="selectOption(option)"
                    class="flex flex-row items-center gap-4 text-3xl py-2 px-4 cursor-pointer">
